...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  var title        = document.getElementById("fileName"),
      categoryInput= document.getElementById("fileCategory"),
      tagsInput     = document.getElementById("fileTags"), // Nuevo campo tags
  ...
  createPost.addEventListener("click", function(e) {
    ...
    var postTitle   = title.value.trim(),
        postContent = content.value.trim(),
    ...
    params.append('action', 'textpad_create_post');
    params.append('nonce', textpad_ajax.nonce);
    params.append('post_title', postTitle);
    params.append('post_content', postContent);
    params.append('category', category);
    params.append('tags', tags);


    // Envía la petición
    fetch(textpad_ajax.ajax_url, {
      method: 'POST',
    ...
    .then(data => {
      if (data.success) {
        smoke.alert("¡Post creado! ID: " + data.data.post_id);
      } else {
        smoke.alert("Error: " + data.data);
      }
    })
    .catch(err => {
      smoke.alert("AJAX error: " + err.message);
    });
  });
