<?php
// Ajusta la ruta hacia wp-load.php según tu estructura
require_once '/../../wp-load.php';

// Si no está logueado, redirige al login de WP
if ( ! is_user_logged_in() ) {
   auth_redirect();
   exit;
}

// Maneja la petición AJAX para crear el post
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'textpad_create_post') {
    // Verifica nonce
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'textpad_create_post')) {
    ...
    // Sanitiza y prepara datos
    $post_title   = sanitize_text_field($_POST['post_title']);
    $raw_content   = sanitize_textarea_field($_POST['post_content']);
    // Convierte saltos de línea en párrafos para Gutenberg
    $paragraph_html = wpautop(esc_html($raw_content));
    // Envuelve en el bloque core/paragraph
    $post_content = '<!-- wp:paragraph -->' . $paragraph_html . '<!-- /wp:paragraph -->';

    $category_slug = sanitize_text_field($_POST['category']);
    $tags_input    = sanitize_text_field($_POST['tags']); // Nuevo campo tags
    ...
    if (!empty($category_slug)) {
        ...
        if ($cat_obj) {
            ...
            $new_cat_id = wp_create_category($category_slug);
            $cat_id = is_wp_error($new_cat_id) ? 1 : $new_cat_id;
        }
    } else {
        $cat_id = 1; // Uncategorized por defecto
    }

    // Inserta el post como borrador
    $new_post = array(
...
<head>
    ...
    // La propia URL de esta página (index.php)
    $ajax_url = esc_url( $_SERVER['REQUEST_URI'] );

    $nonce = wp_create_nonce('textpad_create_post');
    ?>
    <script>
    var textpad_ajax = {
      ajax_url: '<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>',
...
<header role="banner">
    <nav>
        <input id="fileName" type="text" placeholder="Nombre...">
        <input id="fileCategory" type="text" placeholder="Categor&iacute;a (slug)" style="margin-left:10px; width:150px;">
        <input id="fileTags" type="text" placeholder="Tags (entre comas)" style="margin-left:10px; width:150px;">
        <div class="btn-group">
            ...
            <button id="createPost" class="send-btn" title="Publish Post">
...
<textarea id="fileContent" placeholder="Empieza a escribir..." spellcheck="false"></textarea>
